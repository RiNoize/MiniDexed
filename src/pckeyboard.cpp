...
:	CMIDIDevice (pSynthesizer, pConfig, pUI),
	m_pKeyboard (0),
	m_pUI (pUI),
	m_bFoundMessagePending (FALSE),
	m_bNoKeyboardMessageShown (FALSE),
	m_bKeyMessagePending (FALSE),
	m_ucLastRawKey (0),
	m_ucLastNote (0),
	m_nLastNoKeyboardTicks (0)
{
	s_pThis = this;

	memset (m_LastKeys, 0, sizeof m_LastKeys);
	memset (m_DeviceNameFound, 0, sizeof m_DeviceNameFound);
...
void CPCKeyboard::ShowStatus (const char *pLine1, const char *pLine2)
{
	if (m_pUI != 0)
	{
		m_pUI->DisplayWrite ("", pLine1, pLine2, false, false);
	}
}

void CPCKeyboard::Process (boolean bPlugAndPlayUpdated)
{
	// First show pending messages generated by the USB raw-key callback.
	// The display write is done here rather than directly inside the callback,
	// so the keyboard handler stays small and predictable.
	if (m_bKeyMessagePending)
	{
		char Value[17];
		snprintf (Value, sizeof Value, "Raw %02X Note %u", m_ucLastRawKey, m_ucLastNote);
		ShowStatus ("PC Key", Value);
		m_bKeyMessagePending = FALSE;
	}

	if (m_bFoundMessagePending)
	{
		ShowStatus ("PC Keyboard", m_DeviceNameFound);
		m_bFoundMessagePending = FALSE;
	}
	...
	for (unsigned i = 0; i < sizeof DeviceNames / sizeof DeviceNames[0]; i++)
	{
		...
		if (m_pKeyboard != 0)
		{
			...
			strncpy (m_DeviceNameFound, DeviceNames[i], sizeof m_DeviceNameFound - 1);
			m_DeviceNameFound[sizeof m_DeviceNameFound - 1] = '\0';
			m_bFoundMessagePending = TRUE;
			m_bNoKeyboardMessageShown = FALSE;
	...
	// Diagnostic-only status: show that the PC-keyboard object is running,
	// but Circle has not published any ukbdX device. This intentionally
	// overwrites the LCD periodically, so FIX44B is not meant as a final UI fix.
	u64 nNow = CTimer::GetClockTicks ();
	if (   !m_bNoKeyboardMessageShown
	    || nNow - m_nLastNoKeyboardTicks > 5 * CLOCKHZ)
	{
		ShowStatus ("PC Keyboard", "No ukbd dev");
		m_bNoKeyboardMessageShown = TRUE;
		m_nLastNoKeyboardTicks = nNow;
	}
}
	...
	for (unsigned i = 0; i < 6; i++)
	{
		    ...
		    && !FindByte (s_pThis->m_LastKeys, ucKeyCode, 6))
		{
			...
			if (ucKeyNumber != 0)
			{
				...
				s_pThis->m_ucLastRawKey = ucKeyCode;
				s_pThis->m_ucLastNote = ucKeyNumber;
				s_pThis->m_bKeyMessagePending = TRUE;
			}
		}
	}
